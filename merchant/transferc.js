...
function detaillist(){
	...
	if(!confirm("确认导出清分日期为"+tc+"的支付明细单？"))
		return;
	location.href='/sei/settle/api/detailbills?sdate='+tc;			
}
...
function searchlist(op,op1){
	...
	$.ajax({
		url:'/sei/settle/api/clistbills?st='+start+'&li='+pagesize,
		type:'POST',
...
TF.init=function(){
	...
	var str = $("#form").serializeArray(); 
	$.ajax({
		url:'/sei/settle/api/clistbills?st=0&li='+pagesize,
		type:'POST',		
		data:JSON.stringify(convertToJson(str)),		
		contentType:'application/json',
		dataType:'json',		
