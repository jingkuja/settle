...
function backup(){	
	location.href="merchant-list.html?"+urlvalue;	
}
...
function createlist(op){
	...
	etd.innerHTML ="<a href=\"javascript:susp('"+op.id+"');\" class=\"btn btn-small btn-warning\"><span class=\"glyphicon glyphicon-off\"></span>注销</a>&nbsp;&nbsp;<a href=\"javascript:reset('"+op.id+"');\" class=\"btn btn-small btn-default\"><span class=\"glyphicon glyphicon-off\"></span>重置密码</a>" ;
...
function searchlist(op,op1){
	...
	var url='/sei/auth/api/listacct?st='+start+'&li='+pagesize;
	$.ajax({
		url:url,
		type:'POST',
		data:content,		
...
Account.init=function(){
	...
	document.getElementById("aliasid").value=urlvalue;
	}catch (e) {
		// TODO: handle exception
	}
	   
	$.ajax({
		url:'/sei/auth/api/listacct?st=0&li='+pagesize,
		type:'POST',
		data:urlvalue,		
		contentType:'application/json',
		dataType:'json',
		...
		success:function(msg){	
			...
			try{			
				...
				document.getElementById("aliasname").value=op.mname;
			}catch (e) {
	...
	$('#form').validate({
		...
		messages: {
			posnum:{
				required:"登录账号不能为空！",
				maxlength:jQuery.format("登录账号不能大于{0}个字符") 
			},
	...
	$("#submit").bind("click",function(){
		...
		var curl="/sei/auth/api/createAcct";			     	
